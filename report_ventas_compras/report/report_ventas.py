...
class ReportSaleBook(models.AbstractModel):
    ...
    def _get_date(self, data):
        ...
        return 'Del {} al {}'.format(date_from, date_to).upper()
    ...
    def generate_records(self, data):
        ...
        for inv in facturas:
            ...
            linea = {
                ...
                'no': i_ret if retencion else i,
                'retencion': retencion
            }
            if retencion:
                i_ret += 1
                result_ret.append(linea)
            else:
                i += 1
                result_inv.append(linea)
        ...
        no_inv = len(result_inv)
        for index, ret in enumerate(result_ret):
            result_ret[index]['no'] += no_inv
