    ...
    def generate_records(self, data):
        ...
        for inv in facturas:
            ...
            for line in inv.invoice_line_ids:
                ...
                #if inv.journal_id.is_factura_especial == True:
                #    #precio = (line.quantity * line.price_unit) if inv.state != 'cancel' else 0.0
                #    taxes = line.product_id.supplier_taxes_id.compute_all(
                ...
                if line.product_id.tipo_gasto == 'compra':
                    ...
                    if inv.tipo_documento == 'FPC':
                        ...
                        otros_impuestos_s = 0.00
                    elif 'IVA' in line.account_id.name:
                        iva_subtotal += precio
                        iva_servicios += precio
                        print(iva_subtotal, iva_servicios)
                    else:
                        if line.tax_ids:
                            for i in taxes['taxes']:
                                # if all([i['base_code_id'] == base_id[0],
            ...
            subtotal = sum([bienes_gravados, servicios_gravados,
                            ...
                            servicios_i_gravados, combustible_gravado, iva_subtotal, idp])
            if sum([bienes_gravados, servicios_gravados, bienes_pc, servicios_pc, bienes_i_gravados,
                    servicios_i_gravados, combustible_gravado, iva_subtotal]) == 0:
                continue
            ...
            linea = {
                ...
                'tipo': tipo_doc,
                'estado': estado,
                'serie': inv.fel_serie if inv.journal_id.is_factura_especial == True else inv.serie_factura,
                'numero': inv.fel_no if inv.journal_id.is_factura_especial == True else inv.num_factura,
                'origen': "N/A",
            ...
            partners.append(inv.partner_id.name)
            result.append(linea)
        ...
        linea = {
            ...
            'total_pc': total_pc,
            'total_iva': total_iva,
            'total_total': sum([total_g, total_pc, total_iva]),
            'fac_pc': int(fac_pc),
            'fac_c': len(facturas) - fac_pc,
            'fac_total': len(facturas),
            'no_fac': no,
            'no_partners': len(set(partners))
        }
        # result.append(linea)
        # split in folios
        results = []
