...
class ReportJournal(models.AbstractModel):
    ...
    def lines(self, target_move, journal_ids, sort_selection, data):
        ...
        used_rows = []
        for line in lines:
            if row_count in used_rows:
                clean_lines[row_count].append(line)
            else:
                clean_lines[row_count] = [line,]
                used_rows.append(row_count)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        folio_actual = 0
        iteracion = 0
        folio = data['form'].get('folio')
        for journal in data['form']['journal_ids']:
            res[journal] = self.with_context(data['form'].get('used_context', {})).lines(target_move, journal, sort_selection, data)
            if iteracion == 0:
                folio_res[journal] = folio
                folio_actual += len(res[journal])
            else:
                folio_res[journal] = folio_actual
                folio_actual += len(res[journal])
                
            iteracion += 1
            print(len(res[journal]))
